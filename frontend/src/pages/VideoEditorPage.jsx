...
export default function VideoEditorPage() {
  const videoRef = useRef(null), canvasRef = useRef(null), fileInputRef = useRef(null)
  const tlRef = useRef(null), rafRef = useRef(null), trimEndRef = useRef(0), stateRef = useRef({})
  ...
  const reset = () => {
    ...
    setContrast(100); setSaturation(100); setSpeed(1); setAutoError(''); setAutoSuccess('')
  }
  const loadFile = file => {
  ...
  const handleAutoCaption = async () => {
    if (!videoFile) return; setAutoLoading(true); setAutoError(''); setAutoSuccess('')
    try {
      ...
      if (!data.captions?.length) throw new Error('No captions received — does the video have speech?')
      setCaptions(data.captions); setAutoSuccess(`✅ ${data.total_segments} captions generated!`)
  ...
  const handleExport = () => {
    setExporting(true); setExportPct(0); setExportDone(false); let p = 0
    const iv = setInterval(() => {
      p += Math.random() * 12 + 3; setExportPct(Math.min(p, 99))
      if (p >= 99) { clearInterval(iv); setTimeout(() => { setExportPct(100); setExporting(false); setExportDone(true); if (videoUrl) { const a = document.createElement('a'); a.href = videoUrl; a.download = `edited_${videoFile?.name || 'video.mp4'}`; a.click() } }, 600) }
    }, 200)
  }
  ...
  if (!videoUrl) return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', fontFamily: "'DM Sans',sans-serif", padding: '36px' }}>
      <style>{`.ve-drop:hover{border-color:rgba(99,102,241,0.55)!important;background:rgba(99,102,241,0.04)!important;}`}</style>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 40 }}>
        ...
        <div>
          <h1 style={{ fontFamily: 'Syne', fontWeight: 900, fontSize: 22, color: '#f0f0f5', lineHeight: 1 }}>Video Editor</h1>
          <p style={{ color: '#4b5563', fontSize: 12, marginTop: 2 }}>Trim · Text Overlays · Filters · Auto Captions · Export</p>
        </div>
      </div>
        ...
        <div style={{ width: '100%', maxWidth: 680 }}>
          <div className="ve-drop"
            ...
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontFamily: 'Syne', fontWeight: 800, fontSize: 24, color: '#f0f0f5', marginBottom: 8 }}>Drop your video here</p>
              <p style={{ color: '#4b5563', fontSize: 14 }}>or click to browse · MP4, MOV, AVI, WebM</p>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', marginTop: 4 }}>
              {['✂️ Trim & Cut', '📝 Text Overlay', '🎨 Filters', '🎙️ Auto Captions', '⚡ Speed Control', '🎚️ Color Grading'].map(f => (
  ...
  // ─── EDITOR LAYOUT (keeps sidebar visible, fills remaining width) ─────────
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a0f', fontFamily: "'DM Sans',sans-serif", color: '#f0f0f5', overflow: 'hidden' }}>
      <style>{`
        ...
        .vb{cursor:pointer;border:none;transition:all .15s;font-family:'DM Sans',sans-serif;}
        .vb:hover{filter:brightness(1.18);}
        .vb:disabled{opacity:.4;cursor:not-allowed;filter:none;}
        .vtool{cursor:pointer;border:none;transition:all .15s;}
        .vtool:hover{background:rgba(99,102,241,0.1)!important;color:#a5b4fc!important;}
        .rp::-webkit-scrollbar{width:3px;}
        .rp::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.08);border-radius:99px;}
        .vinp::placeholder{color:#374151;}
        .vinp:focus{border-color:rgba(99,102,241,0.5)!important;}
      ...
      <div style={{ height: 50, background: '#0d0d14', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', padding: '0 16px', gap: 10, flexShrink: 0, zIndex: 10 }}>
        ...
        <span style={{ fontFamily: 'Syne', fontWeight: 700, fontSize: 13 }}>Video Editor</span>
        <span style={{ color: 'rgba(255,255,255,0.12)', fontSize: 18 }}>│</span>
        <span style={{ color: '#4b5563', fontSize: 11, maxWidth: 320, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{videoFile?.name}</span>
        <div style={{ flex: 1 }} />
        {[IC.undo, IC.redo].map((ic, i) => (
          <button key={i} className="vb" style={{ width: 30, height: 30, borderRadius: 7, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}>
            <Ico d={ic} s={12} c="currentColor" />
          </button>
        ))}
        <div style={{ width: 1, height: 18, background: 'rgba(255,255,255,0.08)', margin: '0 4px' }} />
        <button className="vb" onClick={() => { setVideoUrl(null); setVideoFile(null) }} style={{ padding: '5px 12px', borderRadius: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#9ca3af', fontSize: 11 }}>New Video</button>
        <button className="vb" onClick={handleExport} disabled={exporting} style={{ padding: '6px 16px', borderRadius: 9, background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', color: '#fff', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, boxShadow: '0 3px 12px rgba(99,102,241,0.35)' }}>
          {exporting ? <><div style={{ width: 11, height: 11, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin .8s linear infinite' }} />{Math.round(exportPct)}%</> : exportDone ? '✅ Downloaded!' : <><Ico d={IC.dl} s={12} c="#fff" />Export</>}
        </button>
      </div>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
          ...
          <div style={{ flex: 1, background: '#060608', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
            ...
            {/* Active filter badge */}
            {activeFilter !== 'None' && (
              <div style={{ position: 'absolute', top: 12, right: 12, background: 'rgba(99,102,241,0.85)', borderRadius: 7, padding: '3px 10px', fontSize: 11, fontWeight: 700, color: '#fff', pointerEvents: 'none' }}>
                {activeFilter}
              </div>
            )}
            ...
            {/* Speed badge */}
            {speed !== 1 && (
              <div style={{ position: 'absolute', top: 12, left: 14, background: 'rgba(245,158,11,0.85)', borderRadius: 7, padding: '3px 10px', fontSize: 11, fontWeight: 700, color: '#fff', pointerEvents: 'none' }}>
                {speed}×
              </div>
            )}
          </div>
            ...
            <div ref={tlRef} onClick={seekTo} style={{ position: 'relative', height: 42, background: 'rgba(255,255,255,0.04)', borderRadius: 9, cursor: 'pointer', overflow: 'visible' }}>
              {duration > 0 && <>
                ...
                <div style={{ position: 'absolute', top: -3, bottom: -3, width: 2, background: '#fff', zIndex: 20, pointerEvents: 'none', left: `${(curTime / duration) * 100}%` }}>
                  <div style={{ width: 9, height: 9, background: '#fff', borderRadius: '50%', marginLeft: -3.5, marginTop: 0 }} />
                </div>
        ...
        <div className="rp" style={{ width: 272, background: '#0d0d14', borderLeft: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column', overflow: 'hidden', flexShrink: 0 }}>
          <div style={{ padding: '12px 16px 10px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <p style={{ fontFamily: 'Syne', fontWeight: 700, fontSize: 11, color: '#4b5563', textTransform: 'uppercase', letterSpacing: 1 }}>{TOOLS.find(t => t.id === activeTool)?.label}</p>
          </div>
          ...
          <div className="rp" style={{ flex: 1, overflowY: 'auto', padding: 14 }}>
            ...
            {activeTool === 'trim' && <div>
              ...
              <div style={{ ...CARD, padding: 10 }}><p style={{ fontSize: 10, color: '#374151', lineHeight: 1.7 }}>💡 Drag the <span style={{ color: '#a5b4fc' }}>purple handles</span> on the timeline or use the sliders.</p></div>
            </div>}
            ...
            {activeTool === 'text' && <div>
              <div style={{ ...CARD, padding: 14, marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 10 }}>
                  {COLORS.map(c => <button key={c} onClick={() => setTxtColor(c)} style={{ width: 22, height: 22, borderRadius: 5, background: c, border: txtColor === c ? '2px solid #fff' : '2px solid transparent', cursor: 'pointer', transition: 'transform .15s', transform: txtColor === c ? 'scale(1.2)' : 'scale(1)' }} />)}
            ...
            {activeTool === 'caption' && <div>
              <div style={{ borderRadius: 12, border: '1px solid rgba(139,92,246,0.3)', background: 'rgba(139,92,246,0.07)', padding: 14, marginBottom: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 8 }}>
                  <div><p style={{ fontSize: 12, fontWeight: 700, fontFamily: 'Syne', marginBottom: 2 }}>🎙️ AI Auto Captions</p><p style={{ fontSize: 10, color: '#a78bfa' }}>Groq Whisper · speech-to-text</p></div>
                  <span style={{ fontSize: 9, padding: '2px 8px', background: 'rgba(16,185,129,0.15)', border: '1px solid rgba(16,185,129,0.3)', color: '#6ee7b7', borderRadius: 100, fontWeight: 700, whiteSpace: 'nowrap' }}>FREE</span>
                </div>
                ...
                <button className="vb" onClick={handleAutoCaption} disabled={autoLoading || !videoFile} style={{ width: '100%', padding: '9px 0', background: 'linear-gradient(135deg,#8b5cf6,#7c3aed)', color: '#fff', borderRadius: 9, fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
                  {autoLoading ? <><div style={{ width: 11, height: 11, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin .8s linear infinite' }} />Analyzing...</> : '🎙️ Generate Captions'}
                </button>
              </div>
            ...
            {activeTool === 'speed' && <div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7, marginBottom: 10 }}>
                {SPEEDS.map(s => <button key={s} className="vb" onClick={() => changeSpeed(s)} style={{ padding: '12px 0', borderRadius: 10, fontSize: 14, fontWeight: 800, fontFamily: 'Syne', background: speed === s ? 'rgba(99,102,241,0.15)' : 'rgba(255,255,255,0.03)', border: `1px solid ${speed === s ? 'rgba(99,102,241,0.4)' : 'rgba(255,255,255,0.07)'}`, color: speed === s ? '#a5b4fc' : '#6b7280' }}>{s}×{s === 1 ? ' ✓' : ''}</button>)}
              </div>
              <div style={{ ...CARD, padding: 10 }}><p style={{ fontSize: 10, color: '#374151', lineHeight: 1.7 }}>💡 <span style={{ color: '#a5b4fc' }}>0.5×</span> slow motion · <span style={{ color: '#a5b4fc' }}>2×</span> double speed</p></div>
            </div>}
          ...
          <div style={{ padding: 14, borderTop: '1px solid rgba(255,255,255,0.07)', flexShrink: 0 }}>
            {exporting && <div style={{ marginBottom: 10 }}>
            ...
            {exportDone && <div style={{ marginBottom: 10, padding: '8px 12px', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.25)', borderRadius: 10 }}><p style={{ color: '#6ee7b7', fontSize: 12, fontWeight: 600 }}>✅ Video exported & downloaded!</p></div>}
