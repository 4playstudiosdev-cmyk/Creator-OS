      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '28px 24px' }}>
        ...
        {view === 'calendar' && (
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            <div style={card}>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
                <button onClick={()=>setCalDate(d=>new Date(d.getFullYear(),d.getMonth()-1,1))}
                  style={{ width:32,height:32,borderRadius:8,background:T.cardAlt,border:`1px solid ${T.border}`,color:T.text,cursor:'pointer',fontSize:16 }}>‹</button>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:4, marginBottom:8 }}>
                {DAYS.map(d => <div key={d} style={{ textAlign:'center', fontSize:11, fontWeight:700, color:T.textMuted, padding:'4px 0', textTransform:'uppercase', letterSpacing:'.06em' }}>{d}</div>)}
              </div>
                  ...
                  return (
                    <div key={day} style={{ minHeight:80, padding:'6px 8px', borderRadius:10, background:isToday?T.goldBg:T.cardAlt, border:`1px solid ${isToday?T.borderGold:T.border}`, cursor:'pointer', position:'relative' }}>
                      ...
                      {dayPosts.slice(0,3).map((post,pi) => {
                        const pc = PLATFORMS.find(p=>p.id===post.platform)?.color || T.gold
                        return (
                          <div key={pi} style={{ fontSize:9, padding:'1px 5px', borderRadius:4, background:`${pc}20`, color:pc, fontWeight:700, marginBottom:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                            {(Array.isArray(post.platforms)?post.platforms[0]:post.platform)||''} · {post.caption?.slice(0,15)||'Post'}
                          </div>
                        )
                      })}
                      {dayPosts.length > 3 && <div style={{ fontSize:9, color:T.textMuted }}>+{dayPosts.length-3} more</div>}
                    </div>
                  )
                })}
              </div>
            ...
            <div style={card}>
              <h2 style={h2}>All Scheduled Posts ({calPosts.filter(p=>p.status==='scheduled').length})</h2>
              {calLoading ? (
                    ...
                    return (
                      <div key={post.id} style={{ padding:'12px 14px', borderRadius:12, background:T.cardAlt, border:`1px solid ${T.border}` }}>
                        <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                          <span style={{ fontSize:11, fontWeight:700, color:pc, background:`${pc}18`, padding:'2px 8px', borderRadius:100 }}>{(Array.isArray(post.platforms)?post.platforms[0]:post.platform)||''}</span>
                        ...
                        <div style={{ fontSize:11, color:T.textMuted }}>
                          📅 {p.scheduled_for || post.scheduled_for ? new Date(p.scheduled_for || post.scheduled_for).toLocaleString('en',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'}) : '—'}
                        </div>
                      </div>
                    )
                  })}
                    ...
                    return (
                      <div key={post.id} style={{ padding:'11px 13px', borderRadius:11, background:T.cardAlt, border:`1px solid ${T.border}` }}>
                        <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5 }}>
                          <span style={{ fontSize:10, fontWeight:700, color:pc }}>{(Array.isArray(post.platforms)?post.platforms[0]:post.platform)||''}</span>
